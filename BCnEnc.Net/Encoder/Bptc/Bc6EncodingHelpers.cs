...
namespace BCnEncoder.Encoder.Bptc
{
	internal static class Bc6EncodingHelpers
	{
		...
		public static ((int, int, int), (int, int, int)) VariateEndpoints1Sub(RawBlock4X4RgbFloat block, (int, int, int) unQuantizedEp0, (int, int, int) unQuantizedEp1, Span<byte> indices, int endpointPrecision, bool signed, float targetError)
		{
			float TryEndpoints((int, int, int) ep0,
				(int, int, int) ep1, Span<byte> indexSpan)
			{
				ep0 = Bc6Block.UnQuantize(ep0, endpointPrecision, signed);
				ep1 = Bc6Block.UnQuantize(ep1, endpointPrecision, signed);

				return FindOptimalIndicesInt1Sub(block, ep0, ep1, indexSpan, signed);
			}

			(int, int, int) WrapEndpoint((int, int, int) ep)
			{
				var mask = ((1 << endpointPrecision) - 1);
				return (
					ep.Item1 & mask,
					ep.Item2 & mask,
					ep.Item3 & mask
			...
			var bestEp0 = FinishQuantizeEndpoint(unQuantizedEp0, endpointPrecision, signed);
			var bestEp1 = FinishQuantizeEndpoint(unQuantizedEp1, endpointPrecision, signed);
			var bestError = TryEndpoints(bestEp0, bestEp1, indices);
			Span<byte> testIndices = stackalloc byte[16];

			var variationAmount = 10;
			while (variationAmount > 0)
			{
				if (bestError < targetError)
				{
					break;
				}
				for (var i = 0; i < ColorVariationGenerator.VarPatternCount;)
				{
					var (testEp0, testEp1) = ColorVariationGenerator.VariateInt(bestEp0, bestEp1, i);
					var tEp0 = WrapEndpoint(testEp0);
					var tEp1 = WrapEndpoint(testEp1);
					var error = TryEndpoints(tEp0, tEp1, testIndices);
					if (error < bestError)
					{
						bestError = error;
						bestEp0 = tEp0;
						bestEp1 = tEp1;
						testIndices.CopyTo(indices);
				...
				variationAmount = (int)MathF.Floor(variationAmount / 2.0f);
			}

			bestEp0 = Bc6Block.UnQuantize(bestEp0, endpointPrecision, signed);
			bestEp1 = Bc6Block.UnQuantize(bestEp1, endpointPrecision, signed);
			return (bestEp0, bestEp1);
		}

		public static ((int, int, int), (int, int, int)) VariateEndpoints2Sub(RawBlock4X4RgbFloat block, (int, int, int) unQuantizedEp0, (int, int, int) unQuantizedEp1,
			Span<byte> indices, int endpointPrecision, int partitionSetId, int subsetIndex, bool signed, float targetError)
		{
			float TryEndpoints((int, int, int) ep0,
				(int, int, int) ep1, Span<byte> indexSpan)
			{
				...
				return FindOptimalIndicesInt2Sub(block, ep0, ep1, indexSpan,
					partitionSetId, subsetIndex, signed);
			}
