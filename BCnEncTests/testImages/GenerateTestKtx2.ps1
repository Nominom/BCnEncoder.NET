...
param (
    ...
    [Parameter(Mandatory=$false)]
    [string]$OutputDir = "./testktx2",
    
    [Parameter(Mandatory=$false)]
    [string]$PVRTexToolPath = "PVRTexToolCLI",
    
    [Parameter(Mandatory=$false)]
    [string]$KtxToolPath = "ktx",
...
if ($Help -or [string]::IsNullOrEmpty($InputImage)) {
    Write-Host "GenerateTestKtx2.ps1 - Generate test KTX2 files for BCnEncoder.NET"
    ...
    Write-Host "  -KtxToolPath      Path to ktx.exe (default: uses ktx from PATH)"
...
# Verify KTX tool is available
try {
    $KtxVersion = Invoke-Expression "& `"$KtxToolPath`" version 2>&1"
    Write-Verbose "Using KTX: $KtxVersion"
} catch {
    Write-Host "Error: KTX tool is not available in the path." -ForegroundColor Red
    Write-Host "Please install it from: https://github.com/KhronosGroup/KTX-Software/releases" -ForegroundColor Red
    exit 1
}
...
$formatMappings = @(
    ...
    @{
        Name = "basisu-etc1s-linear"
        Format = "BASISU_ETC1S,UBN,lRGB"
        ColorSpace = "linear"
        SupportsAlpha = $true
        ExtraOptions = "-q BASISUBEST"
    },
    @{
        Name = "basisu-etc1s-srgb"
        Format = "BASISU_ETC1S,UBN,sRGB"
        ColorSpace = "srgb"
        SupportsAlpha = $true
        ExtraOptions = "-q BASISUBEST"
    },
    @{
        Name = "basisu-uastc-linear"
        Format = "BASISU_UASTC,UBN,lRGB"
        ColorSpace = "linear"
        SupportsAlpha = $true
        ExtraOptions = "-q BASISUBEST"
    },
    @{
        Name = "basisu-uastc-srgb"
        Format = "BASISU_UASTC,UBN,sRGB"
        ColorSpace = "srgb"
        SupportsAlpha = $true
        ExtraOptions = "-q BASISUBEST"
    },
    
    # BC7 format (needs to be transcoded from UASTC)
    @{
        Name = "bc7-linear"
        Format = "BASISU_UASTC,UBN,lRGB"  # Initial format for transcoding
        ColorSpace = "linear"
        SupportsAlpha = $true
        ExtraOptions = "-q BASISUBEST"
    ...
    @{
        Name = "bc7-srgb"
        Format = "BASISU_UASTC,UBN,sRGB"  # Initial format for transcoding
        ColorSpace = "srgb"
        SupportsAlpha = $true
        ExtraOptions = "-q BASISUBEST"
        RequiresTranscode = $true
	...
	@{
        Name = "r5g6b5-linear"
        Format = "R5G6B5,UBN,lRGB"
        ColorSpace = "linear"
        SupportsAlpha = $false
    },
    @{
        Name = "r5g5b5a1-linear"
        Format = "R5G5B5A1,UBN,lRGB"
        ColorSpace = "linear"
        SupportsAlpha = $true
    },
    @{
...
Write-Host "Generating encoded KTX2 files..." -ForegroundColor Cyan
...
foreach ($format in $formatMappings) {
    ...
    $outputFile = Join-Path -Path $OutputDir -ChildPath "$outputName.ktx2"
    ...
    # PVRTexToolCLI seems to assume input is sRGB by default
    ...
    # From memory: BGRA32, RGBA32, BC1 with alpha, BC2, BC3, and BC7 need premultiplied alpha
    $alphaPremulFlag = ""
    ...
    # Transcode to target format if required
    if ($format.RequiresTranscode) {
        $transcodeOutputFile = $outputFile
        $tempUastcFile = $outputFile
        
        # Generate a unique name for the pre-transcoded file
        $preTranscodedFile = "{0}_pretranscoded{1}" -f [System.IO.Path]::GetFileNameWithoutExtension($outputFile), [System.IO.Path]::GetExtension($outputFile)
        $preTranscodedPath = Join-Path -Path $OutputDir -ChildPath $preTranscodedFile
        
        # Make a copy of the pre-transcoded file (UASTC) for later reference PNG generation
        Copy-Item -Path $tempUastcFile -Destination $preTranscodedPath -Force
        
        $transcodeCmd = "& `"$KtxToolPath`" transcode --target $($format.TranscodeTarget) `"$tempUastcFile`" `"$transcodeOutputFile`""
        Write-Verbose "Transcoding to $($format.TranscodeTarget): $transcodeCmd"
        $transcodeOutput = Invoke-Expression "$transcodeCmd 2>&1"
        if ($LASTEXITCODE -ne 0) {
            Write-Host "Error transcoding: $transcodeCmd" -ForegroundColor Red
            Write-Host $transcodeOutput -ForegroundColor Red
        } else {
            Write-Verbose "Transcoded $outputFile to $($format.TranscodeTarget)"
        }
    }
}
...
$totalKtx = @(Get-ChildItem -Path $OutputDir -Filter "*.ktx2" | Where-Object { $_.Name -notlike "*_pretranscoded.ktx2" }).Count
$currentKtx = 0

# Generate reference PNGs for all formats using PVRTexToolCLI's decompression
foreach ($ktxFile in Get-ChildItem -Path $OutputDir -Filter "*.ktx2" | Where-Object { $_.Name -notlike "*_pretranscoded.ktx2" }) {
    $currentKtx++
    ...
    # Check if this is a format that we transcoded and needs special handling
    $format = $formatMappings | Where-Object { $_.Name -eq $ktxName.Replace("$inputBaseName-", "") } | Select-Object -First 1
    $usePreTranscodedFile = $false
    $preTranscodedPath = $null

    if ($format -and $format.RequiresTranscode -eq $true -and (Test-Path (Join-Path -Path $OutputDir -ChildPath "$ktxName`_pretranscoded.ktx2"))) {
        $usePreTranscodedFile = $true
        $preTranscodedPath = Join-Path -Path $OutputDir -ChildPath "$ktxName`_pretranscoded.ktx2"
        Write-Verbose "Using pre-transcoded file for reference PNG generation: $preTranscodedPath"
    }
    
    # Use PVRTexToolCLI to decompress directly to PNG
    $sourceFile = if ($usePreTranscodedFile) { $preTranscodedPath } else { $ktxPath }
    $decompressCmd = "& `"$PVRTexToolPath`" -i `"$sourceFile`" -d `"$refPngPath`" -noout"
    ...
    try {
        ...
        if (Test-Path $refPngPath) {
            ...
            # Delete the pre-transcoded file after reference PNG generation if it exists
            if ($usePreTranscodedFile -and (Test-Path $preTranscodedPath)) {
                Remove-Item -Path $preTranscodedPath -Force
                Write-Verbose "Removed temporary pre-transcoded file: $preTranscodedPath"
            }
        } else {
